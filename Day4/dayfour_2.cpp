...
int main() {
	...
	for (int i = 0; i < boards.size(); i++) winners.push_back(1);

	for (int i = 0; i < numbers.size(); i++) {
		for (int b = 0; b < boards.size(); b++) {
			if (winners[b] && boards[b].MarkNumber(numbers[i])) {
				std::cout << b + 1 << "th board wins with a score of " << boards[b].CalcScore() << std::endl;
				winners[b] = 0;
			}
		}
	}
